...
/**
 * @file Trq.c
 * @brief Torque control and estimation implementation
 *
 * Implements torque PI control and torque calculation algorithms.
 */
...
/**
 * @brief Initialize torque controller (SFO)
 *
 * Configures the torque PI controller with pre-computed gains.
 * Output limits are set to the maximum allowed load-angle [rad-elec].
 *
...
/**
 * @brief Run torque controller at fast ISR rate (ISR0, SFO)
 *
 * Executes the stator-flux-oriented torque control loop:
 * - Looks up MTPV torque limit for the current flux command
 * - Saturates the torque command within ±T_cmd_mtpv
 * - Runs the torque PI to generate a load-angle command (delta_cmd)
 *
 * @param motor_ptr Pointer to motor structure
 */
RAMFUNC_BEGIN
...
/**
 * @brief Calculate electromagnetic torque from QD currents (SFO/RFO)
 *
 * Uses the general salience-aware torque formula:
 * T = 0.75 * P * [lam + (Ld - Lq) * i_d] * i_q [Nm]
 *
 * @param motor_ptr Pointer to motor structure
 * @param i_qd_r    Pointer to QD current vector [A]
 * @param trq       Pointer to output torque result [Nm]
 */
...
/**
 * @brief Reset torque controller and estimated torque filter
 *
 * Resets the torque PI controller state (SFO) and clears the
 * filtered torque estimate used by all control methods.
 *
 * @param motor_ptr Pointer to motor structure
 */
...
/**
 * @brief Run torque observer at fast ISR rate (ISR0)
 *
 * Estimates and filters instantaneous electromagnetic torque:
 * - SFO: transforms currents to stator-flux frame; T_est = 0.75*P*la_d*i_q
 * - RFO (with addon features): T_est = 0.75*P * phi_r * i_q (phi_r from PLL
 *        or flux model)
 * - TBC: T_est = (1/sqrt(2))*P*lam*i_s (scalar, single-phase equivalent)
 * Updates the IIR-filtered torque estimate (T_est_filt) for display and
 * use by the speed controller as a damping feedback signal.
 *
 * @param motor_ptr Pointer to motor structure
 */
RAMFUNC_BEGIN
