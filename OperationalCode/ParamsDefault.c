...
void PARAMS_DEFAULT_InitManual()
{
    ...
    params.sys.fb.hall.track_loop.w0_w = HZ_TO_RADSEC(100.0f); // [Ra/sec]
    params.sys.fb.hall.track_loop.w0_th.min = HZ_TO_RADSEC(10.0f); // [Ra/sec]
    params.sys.fb.hall.track_loop.w0_th.max = HZ_TO_RADSEC(100.0f); // [Ra/sec]
    params.sys.fb.hall.track_loop.tau_ratio = 0.8f; // [sec/sec]=[]
    params.sys.fb.hall.w_zsd_thresh.elec = params.motor.w_nom.elec * (0.10f / 6.0f); // [Ra/sec-elec]
    params.sys.fb.hall.th_r_offset.elec = DEG_TO_RAD(0.0f);
    params.sys.fb.hall.block_comm_offset_comp = Dis; // []

    params.sys.fb.encoder.track_loop.cpr = 1000U; // [#]
    params.sys.fb.encoder.track_loop.w0_w = HZ_TO_RADSEC(100.0f);   // [Ra/sec]
    params.sys.fb.encoder.track_loop.w0_th.min = HZ_TO_RADSEC(1.0f); // [Ra/sec]
    params.sys.fb.encoder.track_loop.w0_th.max = HZ_TO_RADSEC(100.0f); // [Ra/sec]
    params.sys.fb.encoder.track_loop.tau_ratio = 0.8f;  // [sec/sec]=[]
    ...
    params.ctrl.speed.ki_multiple = 1.0f; // [#]
    ...
    params.ctrl.trq.w_p = HZ_TO_RADSEC(60.0f); // [Ra/sec]
    ...
    params.profiler.i_cmd_dc = params.motor.i_cont * 0.4f; // [A]
    params.profiler.i_cmd_ac = params.motor.i_cont * 0.3f; // [A]
    ...
    params.profiler.time_rot_lock = 0.25f; // [sec]
    params.profiler.time_spd = 0.25f; // [sec]
...
void PARAMS_DEFAULT_InitAutoCalc()
{
    ...
    const MINMAX_t Range_F = { 1000.0f, 1500.0f }; // [Hz], excitation
