...
/**
 * @file HallSensor.c
 * @brief Hall sensor implementation
 *
 * Implements Hall sensor processing with coarse position from Hall patterns
 * and fine tracking via adaptive tracking loop. Provides speed estimation
 * from period capture with zero speed detection.
 */
...
/**
 * @brief Hall signal to angle lookup table
 *
 * Maps 3-bit Hall sensor patterns to rotor electrical angles.
 * Valid patterns: 0b101 -> -30°, 0b001 -> +30°, 0b011 -> +90°,
 *                 0b010 -> +150°, 0b110 -> -150°, 0b100 -> -90°
 * Invalid patterns (0b000, 0b111) map to NAN.
 */
 ...
 *
 * Configures speed conversion coefficient, initializes adaptive tracking loop,
 * and sets up zero speed detection timer.
 *
...
/**
 * @brief Reset Hall sensor
 *
 * Resets Hall signal states, period capture, speed sign, tracking loop, and timer.
 *
 * @param motor_ptr Pointer to motor structure
 */
 ...
 *
 * Processes Hall sensor inputs:
 * - Detects invalid Hall patterns (fault)
 * - Updates speed feed-forward from period capture
 * - Detects zero speed condition
 * - Extracts coarse angle from Hall pattern
 * - Determines speed sign from angle transitions
 * - Runs adaptive tracking loop for fine position/speed
 *
 * @param motor_ptr Pointer to motor structure
 */
RAMFUNC_BEGIN
