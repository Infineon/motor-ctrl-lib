 ...
 *
 * Implements the main controller functions for managing speed command integration
 * and rate limiting. Also contains the global controller instances and variable arrays.
 */
 ...
 *
 * Initializes the internal speed command to a specified initial value,
 * accounting for motor direction.
 *
 ...
 *
 * Applies rate limiting to the speed command to ensure smooth acceleration
 * and deceleration. Called at ISR1 rate (fast control loop).
 *
 * @param motor_ptr Pointer to motor structure
 * @param w_target Target speed in electrical/mechanical coordinates
 * @param rate_lim Rate limit factor (per unit, scaled by sampling time)
 */
RAMFUNC_BEGIN
