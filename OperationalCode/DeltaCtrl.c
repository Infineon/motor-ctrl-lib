...
/**
 * @file DeltaCtrl.c
 * @brief Delta (load angle) controller implementation for SFO
 *
 * Implements load angle control for SFO with speed-dependent bandwidth scheduling.
 * The controller adjusts PI gains based on speed to maintain stable control across
 * the operating range.
 */
...
/**
 * @brief Initialize delta controller
 *
 * Configures the PI controller with zero gains (updated at runtime)
 * and sets initial bandwidth reduction coefficient.
 *
...
/**
 * @brief Reset delta controller
 *
 * Resets the PI controller state (note: currently resets torque PI, may be a bug).
 *
 * @param motor_ptr Pointer to motor structure
 */
...
/**
 * @brief Run delta controller (ISR0)
 *
 * Executes load angle control with speed-dependent bandwidth scheduling:
 * - Calculates bandwidth ratio based on speed
 * - Updates PI gains based on current flux and bandwidth
 * - Runs PI controller to generate Q-axis voltage command
 *
 * @param motor_ptr Pointer to motor structure
 */
RAMFUNC_BEGIN
